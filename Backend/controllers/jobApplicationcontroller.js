...
const createApplicationcontroller = async(req,res) => {
    ...
    try{
        const jobPostingavailable = await JobPostingModel.findById(jobID);
...
const getjobseekerapplicationcontroller = async(req,res) => {
    ...
    try{
        const applications = await jobApplicationModel.find({jobseekerID}).populate('jobID').populate('jobseekerID');
...
const getjobapplicationscontroller = async(req,res) => {
    ...
    try{
        const applications = await jobApplicationModel.find({jobID}).populate('jobID').populate('jobseekerID');
        res.status(200).json(applications);
    }
    catch(error){
        res.status(500).json({error: error.message});
